...
class Output(object):
	...
	def filter_and_sort(self, ways):
		...
		# Sort the ways based on slope
		ways = sorted(ways, key=lambda k: k['slope'])
		
		for way in ways:
			if way['slope'] > self.max_slope:
				self.max_slope = way['slope']
...
class TabOutput(Output):
	def output(self, ways):
		...
		print "Slope	Length (mi) Distance (mi)	Id				Name  			County"
		for way in ways:
			print '%d	%9.2f	%9.2f	%10s	%25s	%20s' % (way['slope'], way['length'] / 1609, way['distance'] / 1609, way['id'], way['name'], way['county'])
...
class KmlOutput(Output):
	...
	def get_filename(self, basename):
		filename = basename + '.c_{0:.0f}'.format(self.filter.min_slope)
		if self.filter.max_slope > 0:
			filename += '-{0:.0f}'.format(self.filter.max_slope)
	...
	def get_description(self, way):
		if self.units == 'km':
			return 'Slope: %.2f\nDistance: %.2f km\nType: %s\nSurface: %s' % (way['slope'], way['length'] / 1000, way['type'], way['surface']) 
		else:
			return 'Slope: %.2f\nDistance: %.2f mi\nType: %s\nSurface: %s' % (way['slope'], way['length'] / 1609, way['type'], way['surface']) 
...
# 				sys.stderr.write("\nError: way has no segments: {} \n".format(way['name']))
	...
	def level_for_slope(self, slope):
		if self.filter.min_slope > 0:
			offset = self.filter.min_slope
		else:
			offset = 0
...
class MultiColorKmlOutput(KmlOutput):
	...
	def _write_ways(self, f, ways):
		...
		for way in ways:
			...
			f.write('		<name>' + escape(way['name']) + '</name>\n')
			f.write('		<description>' + self.get_description(way) + '</description>\n')
			current_slope_level = 0
			i = 0
			for segment in way['segments']:
				if segment['slope_level'] != current_slope_level or not i:
					current_slope_level = segment['slope_level']
